...
CREATE OR REPLACE FUNCTION restore_del_for_project_relations() RETURNS trigger AS $$
    ...
    UPDATE TrelloCardUser SET del = FALSE WHERE idCard IN (SELECT id FROM TrelloCards WHERE idList IN (SELECT id FROM Lists WHERE idTrello IN (SELECT id FROM Trellos WHERE idProj = NEW.id))) AND idUser IN (SELECT id FROM Users WHERE del = FALSE);
