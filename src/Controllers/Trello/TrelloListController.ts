...
export class TrelloListController {
    ...
    static async getAllByPositionAndTrello(req: Request, res: Response) {
        try {
    ...
    static async getAllDelByTrello(req: Request, res: Response) {
        try {
            const trelloId = parseInt(req.params.trelloId);
            const lists = await TrelloListDAO.getAllDelByTrello(trelloId);
            const listJson = lists.map((list: any) => list.toJson());
            res.status(200).json(listJson);
        } catch (e) {
            console.error(e);
